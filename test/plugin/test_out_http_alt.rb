...
class HttpAltOutputTest < HttpAltOutputTestBase
  DEFAULT_CONF = %[
  ...
  def test_config_append_tag_to_url
    ...
    assert_equal "http://foo.bar/path?p=", d.instance.endpoint("this.is.test").to_s
    d = create_driver(%[
                      endpoint_url http://foo.bar/path?p=
                      append_tag_to_endpoint_url true
                      ])
    assert_equal "http://foo.bar/path?p=this.is.test", d.instance.endpoint("this.is.test").to_s
  end
  ...
  def test_raise_error_retry
    ...
    d.emit([1, 2], FIXED_TIME)
    d.run
    assert_equal 0, @results.length # was not raise error (no retry)
    logs = d.instance.log.logs
